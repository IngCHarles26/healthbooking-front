...
function NewDate() {
  ...
  const [maxPage, setMaxPage] = useState(
    Math.ceil(doctors.length / cardsPerPage)
  );
  ...
  const filterDoctors = () => {
    let aux = [...doctors];
    if (filtSpecialty)
      aux = [...aux].filter((doctor) => +doctor.Specialty === +filtSpecialty);
    if (filtSure)
      aux = [...aux].filter((doctor) => doctor.Sures.includes(+filtSure));
    aux = [...aux].filter((doctor) =>
      doctor.name.toLowerCase().includes(filtName.toLowerCase())
    );
    if (orderName) {
      aux = [...aux].sort((a, b) =>
        orderName === "a-z"
  ...
  const handleHour = (val) => {
    if (typeof val === "number")
      setHourSelected(val === hourSelected ? 0 : val);
          ...
          <header>
            ...
            <div className="filters-group">
              <div className="options">
                <div onClick={() => setOrderName("")}>
                  {/*agregar aqui el evento on Click */}
                  <img src={deleteOrder} alt="delete " />
                </div>

                <form>
                  <label htmlFor="order">
                    Sort <img src={orderCards} alt="orderCards" />
                  </label>
                  <select
                    id="order"
                    value={orderName}
        ...
        <section className="agenda-doctor">
          <table className="table-disponibility">
            <thead>
              <tr>
                {agenda[0]
                  .slice(8 * (agendaPage - 1), 8 * agendaPage)
                  .map((el, ix) => (
                    <th key={"heandTable_" + ix}>{el}</th>
                  ))}
              </tr>
            </thead>
  ...
  const handleDoctorSelection = (id) => {
    ...
    if (id) {
      healthApi
        ...
        .then(({ data }) => {
          dispatch(addDataAgenda(getPlan(data)));
        })
...
function SpecialtyOption(props) {
  ...
  return (
    <li
      className={style}
      onClick={() => setFiltSpecialty(index == filtSpecialty ? "" : index)}
    >
      {name}
    </li>
  );
...
function ObraSocial(props) {
  ...
  return <option value={ix}>{name}</option>;
}
